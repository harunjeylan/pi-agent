...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const projectRulesPath = path.join(ctx.cwd, ".pi", "control-rules.yaml");
    const globalRulesPath = path.join(agentPath, "control-rules.yaml");
    ...
    try {
      if (rulesPath) {
        ...
        ctx.ui.notify(
          "🛡️ Damage-Control: No rules found at .pi/control-rules.yaml (project or global)",
        );
      }
    } catch (err) {
      ctx.ui.notify(
